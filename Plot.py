...
from mathgraph3D.Shapes import Shape;
from mathgraph3D.Color import ColorStyle, Styles, preset_styles;
...
from mathgraph3D.OtherCoordinateSystems import CylindricalFunction, SphericalFunction;
from mathgraph3D.ImplicitPlots import ImplicitSurface
##import numpy as np;
...
class Plot():
    ...
    def __init__(self, surface, gui=None, x_start=-4, x_stop=4, y_start=-4, y_stop=4, z_start=-4, z_stop=4, background=(255, 255, 255), z_axis_length=200, y_axis_length=200, x_axis_length=200,
        ...
        self.x_axis_length, self.y_axis_length, self.z_axis_length = x_axis_length, y_axis_length, z_axis_length;
        ...
        self.ticks = ticks;
        self.functions, self.points, self.shapes, self.clipping_planes = [], [], [], [];
    ...
    def set_bounds(self, x_start, x_stop, y_start, y_stop, z_start, z_stop, anch=False):
        ...
        self.x_scale = 2 * self.x_axis_length / self.units_x
        self.y_scale = 2 * self.y_axis_length / self.units_y
        self.z_scale = 2 * self.z_axis_length / self.units_z
        if anch: self.regenerate_all_polygons()
        self.needs_update = True

    def regenerate_all_polygons(self):
        """ Regenerate the polygons of all objects in the Plot """
        for function in self.functions:
            if isinstance(function, (ImplicitSurface, Function3D)):
    ...
    def get_point_coordinates(self, x, y, z):
        ...
        return (x * self.x.x + y * self.y.x + z * self.z.x,
...
##        dist = sqrt(distance3D(self.sortbox, (x, y, z))/25)
##        #print(dist)
##        x /= dist
##        y /= dist
##        z /= dist
...
##        self.z = Vector(0, self.z_scale * cos(self.beta));
        ...
        a, b, c = self.alpha, self.beta, self.c;

        xx = cos(c)*cos(a)-sin(c)*sin(a)*sin(b);
        xy = cos(c)*sin(a)*sin(b)+sin(c)*cos(a);
        yx = -cos(c)*sin(a)-sin(c)*cos(a)*sin(b);
        yy = cos(c)*cos(a)*sin(b)-sin(c)*sin(a);
        zx = -sin(c)*cos(b);
        zy = cos(c)*cos(b);
        self.x = Vector(self.x_scale * xx, self.x_scale * xy);
        self.y = Vector(self.y_scale * yx, self.y_scale * yy);
        self.z = Vector(self.z_scale * zx, self.z_scale * zy);
##        self.x = Vector(xx, xy);
##        self.y = Vector(yx, yy);
##        self.z = Vector(zx, zy);
...
##
##        self.sortbox = (x, y, z);
        theta = -(pi / 2 + self.alpha)
        r = 100
        if pi/2<=self.beta<=3/2*pi:
            r *= -1
...
##            y = r * sin(theta)
    ...
    def point(self, point, size=None):
        """ draw a point """
...
##        print("{}(x-{}) + {}(y-{}) + {}(z-{})".format(a, x1, b, y1, c, z1)); # print the standard form
    ...
    def draw_axes(self):
        ...
        for y in drange(self.y_start, self.y_stop + p, p):
            ...
            if y and y % self.line_number_freq == 0:
                if self.ticks:
                    self.add_shape((0, y, 0), pygame.draw.line, self.surface, self.y_axis_color, self.screen_point(-0.1, y, 0), self.screen_point(0.1, y, 0), self.axis_weight);
                if self.line_numbers:
        ...
        for z in drange(self.z_start, self.z_stop + p, p):
            ...
            if z and z % self.line_number_freq == 0:
                if self.ticks:
                    self.add_shape((0, 0, z), pygame.draw.line, self.surface, self.z_axis_color, self.screen_point(0, -0.1, z), self.screen_point(0, 0.1, z), self.axis_weight);
                if self.line_numbers:
    ...
    def draw_line(self, M, color, p1, p2, weight):
        """ draw a line easily """
        self.add_shape(M, pygame.draw.line, self.surface, color, self.screen_point(*p1), self.screen_point(*p2), weight);
    ...
    def zoom(self, f):
        ...
        self.x_axis_length = max(0, self.x_axis_length+f);
        self.y_axis_length = max(0, self.y_axis_length+f);
        self.z_axis_length = max(0, self.z_axis_length+f);
        self.needs_update = True;
        self.set_bounds(self.x_start, self.x_stop, self.y_start, self.y_stop, self.z_start, self.z_stop);

    def clip(self, polygons):
        """ Clip the polygons according to the Plot's set of clipping planes """
        polys = []
        for polygon in polygons:
            for plane in self.clipping_planes:
                polygon = plane.clip_polygon(polygon)
                if polygon is None: break
            if polygon is not None: polys.append(polygon)
        return polys

    def add_clipping_plane(self, plane):
        self.clipping_planes.append(plane)
        self.regenerate_all_polygons()
